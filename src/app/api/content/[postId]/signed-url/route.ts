 ...
 *
 * IMPORTANTE: usamos el cliente ADMIN (service role) para leer el post y generar
 * la Signed URL, porque el bucket "content" es privado y no tiene una policy de
 * SELECT para el cliente anonimo/autenticado normal. El control de acceso real
 * ya lo hicimos arriba a mano con hasAccessToPost(), asi que es seguro bypassear
 * RLS aca adentro.
 */
export async function GET(
  ...
  try {
    ...
    // media_url guarda el path dentro del bucket "content", ej: "{creatorId}/foto1.jpg"
    ...
    if (signError || !signed) {
      return NextResponse.json(
        { error: "No se pudo generar la URL del contenido" },
        { status: 500 }
      );
    }
